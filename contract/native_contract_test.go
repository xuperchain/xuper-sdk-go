...
func TestNativeInvoke(t *testing.T) {
	...
	mName := "Increase"
	r, e := nativeContract.InvokeNativeContract(mName, args,"15587517")
	if e != nil {
		t.Error(e)
	}
...
func TestNativeQuery(t *testing.T) {
	...
	methodName := "Increase"
	preExeRPCRes, e := NativeContract.QueryNativeContract(methodName, args)			// methodName == "" 如何处理
	if e != nil {
		t.Error(e)
	}
